...
for i = 1:length(pop)
    pop(i).lambdaInitial = lambda(i);
    pop(i).muInitial = mu(i);
end
%%原始参数设置
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%设置每个栖息地的各自迁入迁出概率
    for i = 1:length(pop)
        pop(i).lambda = lambdaFactor ^ (i - 1) * pop(i).lambdaInitial;
        pop(i).mu = muFactor ^ (length(pop) - i) * pop(i).muInitial;
    end
    ...
    for i = 1:length(pop)
        if pop(i).Cost > costThreshold
            pop(i).pMutation = pMutationInitial * 1.2; % 适应度差的栖息地变异率加da
        else
            pop(i).pMutation = pMutationInitial;
        end
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    newpop=pop;
