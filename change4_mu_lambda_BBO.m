function [Leader_score,Leader_pos,Convergence_curve1,Convergence_curve2,Convergence_curve3]=change4_mu_lambda_BBO(pop,nPop,MaxIt,varMin,varMax,nVar,ComputeCost1,ComputeCost2,ComputeCost3)
...
for i=1:nPop
    %初始化计算种群内个体的总代价
    pop(i).Cost = m1*(Max_Cost1-pop(i).Cost1)/(Max_Cost1-100) + m2*(Max_Cost2-pop(i).Cost2)/(Max_Cost2-1200) + m3*(Max_Cost3-pop(i).Cost3)/(Max_Cost3-2100);
end
...
%        fitnessValues = [pop.Cost]; 
        ...
        for i = 1:nPop
            distanceToPosIdeal = sqrt(sum((pop(i).Position - PosIdealPoint).^2));
            distanceToNegIdeal = sqrt(sum((pop(i).Position - NegIdealPoint).^2));
...
for i = 1:nPop
    for j = i+1:nPop
        ...
        if dist < Remove_Threshold
            p1 = local_search(CostFunction1,newpop(j).Position,VarMin,VarMax);
            p2 = local_search(CostFunction2,newpop(j).Position,VarMin,VarMax);
            p3 = local_search(CostFunction3,newpop(j).Position,VarMin,VarMax);
            ...
            endCost1 = m1*(Max_Cost1-numCost1)/(Max_Cost1-100) + m2*(Max_Cost2-numCost2)/(Max_Cost2-1200) + m3*(Max_Cost3-numCost3)/(Max_Cost3-2100);
            
            numCost1=CostFunction1(p2);
            numCost2=CostFunction2(p2);
            numCost3=CostFunction3(p2);
            
            endCost2 = m1*(Max_Cost1-numCost1)/(Max_Cost1-100) + m2*(Max_Cost2-numCost2)/(Max_Cost2-1200) + m3*(Max_Cost3-numCost3)/(Max_Cost3-2100);

            numCost1=CostFunction1(p3);
            numCost2=CostFunction2(p3);
            numCost3=CostFunction3(p3);
            
            endCost3 = m1*(Max_Cost1-numCost1)/(Max_Cost1-100) + m2*(Max_Cost2-numCost2)/(Max_Cost2-1200) + m3*(Max_Cost3-numCost3)/(Max_Cost3-2100);
            tcost = endCost1;
            tindex = 1;
            if tcost < endCost2
                tcost = endCost2;
...
%             pop(j).Cost = pcost;
    ...
    if similarityCount > nPop * probability  % 如果种群中超过 85% 的个体对相似
...
%         newpop(i).Cost=CostFunction(newpop(i).Position);
        ...
        newpop(i).Cost = m1*(Max_Cost1-newpop(i).Cost1)/(Max_Cost1-100) + m2*(Max_Cost2-newpop(i).Cost2)/(Max_Cost2-1200) + m3*(Max_Cost3-newpop(i).Cost3)/(Max_Cost3-2100);   
    ...
    %%利用理想点计算适应度
    for i = 1:nPop
        distanceToPosIdeal = sqrt(sum((newpop(i).Position - PosIdealPoint).^2));
        distanceToNegIdeal = sqrt(sum((newpop(i).Position - NegIdealPoint).^2));
        fitnessValues(i) = distanceToPosIdeal / (distanceToNegIdeal + distanceToPosIdeal); % 更新适应度
    end
%     disp(fitnessValues);
    % Sort New Population
...
%     % Select Next Iteration Population  精英策略
    ...
    disp(['Iteration ' num2str(it) ': c4Best Cost = ' num2str(BestCost(it)) ', Average Cost = ' num2str(avgCost)]);
